...
- For the dashboard scenes: a Neo4j instance with a populated graph on `127.0.0.1:7687`
  (`docker compose -f examples/docker-compose.yml up -d` gives you one) and an `audit.env` at
  the repo root holding its credentials. The dashboard is deliberately started from
  `audit.env` rather than `.env`, so whatever real hosts your own `.env` names can never be
  reached by a recording run. `audit.env` is gitignored by the `*.env` rule.
