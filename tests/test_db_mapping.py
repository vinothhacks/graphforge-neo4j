...
def test_column_mapping_bool():
    mx = get_extractor("mysql", host="h", port=3306, user="u", password="")
    col = mx.map_column({"table_name": "t", "name": "c", "ordinal": 1,
...
def test_write_database_emit(tmp_path):
    ...
    sm.columns = [
        {"table": "orders", "name": "id", "ordinal": 1, "dataType": "int",
         "isNullable": False, "columnKey": "PRI", "default": "", "extra": ""},
        {"table": "orders", "name": "cid", "ordinal": 2, "dataType": "int",
         "isNullable": False, "columnKey": "MUL", "default": "", "extra": ""},
    ]
    sm.indexes = [{"name": "PRIMARY", "table": "orders", "isUnique": True,
                   "indexType": "BTREE", "columns": ["id"]}]
    sm.procedures = [{"name": "sp_recalc", "type": "PROCEDURE",
                      "definition": "BEGIN UPDATE orders SET total=0; END",
                      "parameters": ["IN cid int"]}]
    sm.foreign_keys = [{"constraintName": "fk", "fromTable": "orders",
                        "fromColumn": "cid", "toTable": "cust", "toColumn": "id"}]
    meta.schemas = [sm]
...
def test_procedure_mapping_includes_definition():
    mx = get_extractor("mysql", host="h", port=3306, user="u", password="")
    proc = mx.map_procedure({"name": "sp_x", "routine_type": "PROCEDURE",
                             "definition": "SELECT 1"})
