...
class GitIngestor:
    ...
    def ingest(
        ...
        for spec in _progress(repo_specs, desc="repos", unit="repo"):
            ...
            try:
                s = self.ingest_repo(spec, include_lines, with_structure, with_history, replace)
                stats["repos"] += 1
    ...
    def ingest_repo(
        ...
        if with_structure:
            stats["files"] = self._ingest_structure(repo, path, include_lines)
        if with_history:
            stats["commits"] = self._ingest_history(repo, path)

        # record completion status + counts on the Repository node
        self.writer.write([Operation(
            "MATCH (r:Repository {id: $id}) "
            "SET r.status = $status, r.lastIngestedAt = $at, r.files = $files, r.commits = $commits",
    ...
    # -- structure ---------------------------------------------------------
    def _ingest_structure(self, repo: str, path: str, include_lines: bool) -> int:
        data = scan_mod.scan_repo(path, repo, include_lines)
    ...
    def _file_ops(self, repo, f: scan_mod.ScannedFile, module_name_by_key, include_lines) -> List[Operation]:
        ...
        if f.java:
            ops += self._java_ops(repo, fid, f)
        if include_lines and f.lines:
            ops += self._line_ops(fid, repo, f)
        return ops

    def _java_ops(self, repo, fid, f) -> List[Operation]:
        ops: List[Operation] = []
        pkg = f.package
        owned_fqns: List[str] = []
        for kind_key, decl_type in (("classes", "class"), ("interfaces", "interface"), ("enums", "enum")):
            for c in f.java.get(kind_key, []):
                fqn = f"{pkg}.{c['name']}" if pkg else c["name"]
                ...
                ops.append(merge_node("Class", {"id": cid}, {
                    ...
                    "type": decl_type, "visibility": c.get("visibility", "public"),
                ...
                # promote framework classes to a semantic label (Entity/ManagedBean/EJBBean)
                if c.get("stereotype"):
        ...
        # methods attach to the first owned type in the file (best effort)
        if owned_fqns:
            owner = owned_fqns[0]
            ocid = _class_id(repo, owner)
            for meth in f.java.get("methods", []):
                ...
                ops.append(merge_rel(NodeRef("Class", {"id": ocid}), "HAS_METHOD",
                                     NodeRef("Method", {"id": mid})))
        # imports: file's owned classes IMPORT external fqns
        for imp in f.java.get("imports", []):
    ...
    # -- history -----------------------------------------------------------
    def _ingest_history(self, repo: str, path: str) -> int:
        h = history_mod.extract_history(path, self.gs.history_limit)
