...
def _db_id(engine: str, host: str, name: str) -> str: return f"{engine}://{host}/{name}"
def _schema_id(db_id: str, schema: str) -> str: return f"{db_id}/{schema}"
def _table_id(schema_id: str, table: str) -> str: return f"{schema_id}/{table}"
def _column_id(table_id: str, column: str) -> str: return f"{table_id}#{column}"
def _index_id(table_id: str, index: str) -> str: return f"{table_id}!{index}"
def _view_id(schema_id: str, view: str) -> str: return f"{schema_id}/view/{view}"
def _proc_id(schema_id: str, proc: str) -> str: return f"{schema_id}/proc/{proc}"
...
class DbIngestor:
    ...
    def ingest_sources(self, sources: List[dict], replace: bool = False) -> Dict[str, int]:
        """Each source: {engine, host, port, user, password, [databases], [driver], [schemas]}.
        ...
        for src in sources:
            extractor = get_extractor(
                src["engine"], host=src.get("host", "127.0.0.1"), port=src.get("port", 0),
                user=src.get("user", ""), password=src.get("password", ""),
                driver=src.get("driver"), schemas=src.get("schemas"),
            )
            databases = [d for d in (src.get("databases") or []) if d]
            if not databases:
                try:
                    databases = extractor.list_databases()
                    log.info("auto-discovered %d database(s) on %s", len(databases), src.get("host"))
                except Exception as exc:  # discovery failed — skip this source, keep going
                    log.error("could not list databases on %s: %s", src.get("host"), exc)
                    failures.append({"source": f"{src.get('engine')}://{src.get('host')}", "error": str(exc)})
                    continue
            ...
            for database in _progress(databases, desc="databases", unit="db"):
                ...
                except Exception as exc:  # one bad database must not abort the rest
        ...
        if failures:
            ...
            log.warning("%d item(s) failed: %s", len(failures),
    ...
    def write_database(self, meta: DatabaseMeta) -> Dict[str, int]:
        ...
        for sm in meta.schemas:
            ...
            ops: List[Operation] = [
                ...
                merge_rel(NodeRef("Database", {"id": db_id}), "HAS_SCHEMA",
                          NodeRef("Schema", {"id": schema_id})),
            ]
            for t in sm.tables:
                tid = _table_id(schema_id, t["name"])
                ops.append(merge_node("Table", {"id": tid},
                                      {"name": t["name"], "schema": sm.name, "database": meta.name}))
                ops.append(merge_rel(NodeRef("Schema", {"id": schema_id}), "HAS_TABLE",
            ...
            for c in sm.columns:
                ...
                ops.append(merge_node("Column", {"id": cid}, {
                    "name": c["name"], "table": c["table"], "schema": sm.name, "database": meta.name,
                    "ordinal": c.get("ordinal"), "dataType": c.get("dataType", ""),
                    "isNullable": c.get("isNullable", True), "columnKey": c.get("columnKey", ""),
                    "default": c.get("default", ""), "extra": c.get("extra", ""),
                }))
                ops.append(merge_rel(NodeRef("Table", {"id": tid}), "HAS_COLUMN",
                                     NodeRef("Column", {"id": cid})))
                counts["columns"] += 1

            for ix in sm.indexes:
                tid = _table_id(schema_id, ix["table"])
                iid = _index_id(tid, ix["name"])
                ops.append(merge_node("Index", {"id": iid}, {
                    "name": ix["name"], "table": ix["table"], "database": meta.name,
                    "isUnique": ix.get("isUnique", False), "indexType": ix.get("indexType", ""),
                    "columns": ix.get("columns", []),
                }))
                ops.append(merge_rel(NodeRef("Table", {"id": tid}), "HAS_INDEX",
                                     NodeRef("Index", {"id": iid})))

            for v in sm.views:
                vid = _view_id(schema_id, v["name"])
                ops.append(merge_node("View", {"id": vid},
                                      {"name": v["name"], "database": meta.name, "definition": v.get("definition", "")}))
                ops.append(merge_rel(NodeRef("Schema", {"id": schema_id}), "HAS_VIEW",
                                     NodeRef("View", {"id": vid})))

            for p in sm.procedures:
                pid = _proc_id(schema_id, p["name"])
                ops.append(merge_node("StoredProcedure", {"id": pid}, {
                    "name": p["name"], "database": meta.name,
                    "type": p.get("type", "PROCEDURE"),
                    "definition": p.get("definition", ""),
                    "parameters": p.get("parameters", []),
                }))
                ops.append(merge_rel(NodeRef("Schema", {"id": schema_id}), "HAS_PROCEDURE",
            ...
            for fk in sm.foreign_keys:
                ...
                ops.append(merge_node("Table", {"id": to_tid},
                                      {"name": fk["toTable"], "schema": sm.name, "database": meta.name}))
                ops.append(merge_node("Column", {"id": to_cid},
                                      {"name": fk["toColumn"], "table": fk["toTable"], "schema": sm.name}))
                ops.append(merge_rel(NodeRef("Column", {"id": from_cid}), "FOREIGN_KEY",
                                     NodeRef("Column", {"id": to_cid}),
                                     {"constraintName": fk.get("constraintName", "")}))
                ops.append(merge_rel(NodeRef("Table", {"id": from_tid}), "REFERENCES",
