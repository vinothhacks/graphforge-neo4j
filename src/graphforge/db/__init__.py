...
def get_extractor(engine: str, **kwargs) -> SchemaExtractor:
    ...
    if key not in _ENGINES:
        raise ValueError(f"unsupported db engine {engine!r}; choose from {sorted(set(_ENGINES))}")
...
def parse_db_url(url: str) -> dict:
    ...
    if engine not in _ENGINES:
        raise ValueError(
            f"unsupported db url scheme {p.scheme!r}; use one of {sorted(set(_ENGINES))}"
        )
